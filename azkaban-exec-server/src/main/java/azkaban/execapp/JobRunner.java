...
public class JobRunner extends EventHandler implements Runnable {
    ...
    public JobRunner(final ExecutableNode node, final File workingDir, final ProjectLoader projectLoader,
                     final ExecutorLoader loader, final JobTypeManager jobtypeManager, final Props azkabanProps) {
            ...
            if (this.props.containsKey("user.to.proxy")) {
                ...
                if (this.proxyUsers != null && !this.proxyUsers.contains(jobProxyUser)) {
                    this.logger.error("User " + jobProxyUser
                            + " has no permission to execute this   " + this.jobId + "!");
                    return null;
                }
            }
    ...
    private Status runJob() {
        ...
        try {
            ...
            // 作业失败，检测是否发送报警
            if (node.getInputProps().containsKey("warn.level"))
                sendWarningCheck(node.getInputProps().getInt("warn.level"), node.getRetries(), node.getAttempt(), e);
    ...
    // 报警级别、重试次数、当前第几次重试、出错的对象
    // attempt=0，本身报错；1以及以后的数字，重试报错；attempt == retries，最后一次
    private void sendWarningCheck(int warnLevel, int retries, int attempt, Throwable e) {
        // 不报警
        if (warnLevel == 4)
            return;

        // 如果是级别中：如果不是第一次，则不报警
        if (warnLevel == 2 && attempt > 0)
            return;

        // 如果是级别低：如果不是最后一次，则不报警
        if (warnLevel == 3 && attempt < retries)
            return;

        sendWarning(retries, attempt, e);
    }

    // 报警级别、重试次数、当前第几次重试、出错的对象
    private void sendWarning(int retries, int attempt, Throwable e) {
        if (!node.getInputProps().containsKey("notify.user")) {
            serverLogger.error("作业没有配置报警用户 " + jobId);
            return;
        }

        String notifyUser = node.getInputProps().getString("notify.user");
        if (org.apache.commons.lang.StringUtils.isBlank(notifyUser)) {
            serverLogger.error("作业配置的报警用户为空 " + jobId);
            return;
        }

        User user = projectLoader.selectUser(notifyUser);
        if (user == null) {
            serverLogger.error("用户不存在 " + notifyUser);
            return;
        }

        // 发送邮件
        //Emailer mailAlerter = new Emailer(azkabanProps);
        // 发送短信-调用公司短信接口
        Phoner.sendMessage(user.getPhone(), "作业 " + this.jobId + " 执行出错！");
    }
}
